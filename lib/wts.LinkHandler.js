...
var escapeWikiLinkContentString = function(linkTarget, state, contentNode) {
	...
	var res = state.serializer.wteHandlers.escapeWikiText(state, linkTarget, { node: contentNode });
	state.inLink = false;
...
var linkHandler = function(node, state, cb) {
	...
	if ( linkData.type !== null && linkData.target.value !== null  ) {
		...
		if (/\b(mw:ExtLink|mw:PageProp\/Language)\b/.test(linkData.type)) {
			...
			if (interWikiMatch &&
				...
				if (oldPrefix && (
						...
						(env.conf.wiki.interwikiMap[oldPrefix[1].toLowerCase().replace(/^:/, '')] || {}).url ===
						(env.conf.wiki.interwikiMap[interWikiMatch[0].toLowerCase().replace(/^:/, '')] || {}).url
						))
				{
		...
		if (/^mw:WikiLink$/.test( linkData.type ) ||
					...
					(target.modified || linkData.contentModified || dp.stx === 'url'))
			{
				...
				if (magicLinkMatch) {
					...
					cb(serializer(magicLinkMatch, target.value, contentStr), node);
					return;
			...
			return;
		} else if ( /(?:^|\s)mw:(Image|Video)/.test(linkData.type) ) {
		...
		if ( isComplexLink ( node.attributes ) ) {
			...
			var hrefStr = escapeExtLinkURL(node.getAttribute('href'));
			cb( '[' + hrefStr + ' ' +
				state.serializeLinkChildrenToString(node, this.wteHandlers.aHandler, false) +
				']', node );
			return;
...
var figureHandler = function(node, state, cb) {
	...
	if (sizeUnmodified && upright
			...
			&& type in {'mw:Image/Frameless':1, 'mw:Image/Thumb':1, 'mw:Video/Frameless':1, 'mw:Video/Thumb':1})
	{
