...
var escapeWikiLinkContentString = function(linkTarget, state, contentNode) {
	...
	var res = state.serializer.escapeWikiText(state, linkTarget, { node: contentNode });
	state.inLink = false;
	...
	if (!state.env.isValidLinkTarget(linkTarget) || /[\|]/.test(linkTarget)) {
		...
		// What would this be?
		// FIXME: VE doesn't provide invalid link targets, so there is no
		// real WYSIWYG concern here. It should be fine to just leave this
