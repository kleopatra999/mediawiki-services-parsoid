...
TemplateHandler.prototype._onChunk = function( state, cb, chunk ) {
	...
	if (this.options.wrapTemplates) {
		if (!state.emittedFirstChunk && !this.options.inTemplate) {
			...
			/* TODO: Move param conversion to HTML to an independent method so
			 * it can be used by the onDocument path when we enable it.
			 */
...
TemplateHandler.prototype.onTemplateArg = function (token, frame, cb) {
	...
	if (this.options.wrapTemplates) {
		...
		newCB = function(res) {
			...
			toks[0].attribs.push(new KV("data-mw-arginfo", JSON.stringify(tplHandler.getArgInfo(state))));
