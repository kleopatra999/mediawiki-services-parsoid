...
TemplateHandler.prototype.resetState = function(opts) {
	this.atTopLevel = opts && opts.toplevel;
};

TemplateHandler.prototype.register = function() {
...
TemplateHandler.prototype.onTemplate = function ( token, frame, cb ) {
	...
	// Store information about template arguments inside the frame so they can be accessed later
	// in <infobox> extension
	frame.templateArgInfo = frame.templateArgInfo || {};
	if(tgt && tgt.target) {
		//TODO: this solution is a hot fix for https://wikia-inc.atlassian.net/browse/DAT-3232
		//TODO: we should figure out how to refactor this code - https://wikia-inc.atlassian.net/browse/DAT-3251
		// infobox template can be used multiple times in single article
		// and we need to store args for each infobox template invocation
		if (!frame.templateArgInfo[tgt.target]) {
			frame.templateArgInfo[tgt.target] = []
		}

		frame.templateArgInfo[tgt.target].push(this.getArgInfo(state));
...
TemplateHandler.prototype.checkForMagicWordVariable = function(tplToken) {
	...
	if ( name === "!" ) {
		...
		// If we're not at the top level, return a table cell. This is always
...
TemplateHandler.prototype.getArgInfo = function (state) {
	...
	for (var i = 1, n = params.length; i < n; i++) {
		...
		kWt = kSrc.trim();
		k = Util.tokensToString(params[i].k, true, true);
		if (Array.isArray(k)) {
