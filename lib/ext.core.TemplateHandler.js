	...
	function resolvabilityInfo( tokens ) {
		...
		if ( Array.isArray(maybeTarget) ) {
			...
			for ( var i = 0, l = tgtTokens.length; i < l; i++ ) {
				var ntt = tgtTokens[0];
				// We are okay with {{..}} and {{{..}}} in template targets.
				if (ntt.constructor === SelfclosingTagTk) {
					allString = false;
					if (ntt.name !== 'template' && ntt.name !== 'templatearg') {
						return { isStr: false };
					}
	...
	Util.expandParserValueValues (
			...
			function ( expandedAttrs ) {
				...
				if ( self.options.wrapTemplates ) {
					...
					// tokens[0].attribs.push(new KV("data-mw-arginfo", JSON.stringify(this.getArgInfo(state))));
					tokens.push(self.getEncapsulationInfoEndTag(state));
					// console.log("convertAttribsToString, tokens:", tokens);
				}
				cb( { tokens: tokens } );
			}
	);
	...
	if ( resolvedTgt.isPF ) {
		...
		if (!this.parserFunctions[target]) {
			...
			if (this.options.wrapTemplates) {
				...
				// res[0].attribs.push(new KV("data-mw-arginfo", JSON.stringify(this.getArgInfo(state))));
				res.push(this.getEncapsulationInfoEndTag(state));
				// console.log("_expandTemplate, res:", res);
			}
			cb( { tokens: res } );
			return;
		}
...
TemplateHandler.prototype._onChunk = function( state, cb, chunk ) {
	...
	// console.log("\n_onChunk", this.counter, ", options:", this.options);
	// console.log( "_onChunk", this.counter, ", state:", state );
	// console.log( "_onChunk", this.counter, ", token:", JSON.stringify(state.token, null, " ") );
	// console.log( "_onChunk", this.counter, ", chunk:", chunk );
	var i, n;
	...
	if (this.options.wrapTemplates) {
		if (!state.emittedFirstChunk && !this.options.inTemplate) {
			...
			if (state.recordArgDict) {
				...
				// console.warn("token: " + JSON.stringify(state.token));
				// console.log("argInfo", this.counter, ":" + JSON.stringify(argInfo, null, " "));
				...
				var getParamHTML = function (paramInfo, eachCb) {
					...
					// console.log("paramInfo", this.counter, ":", paramInfo);
					// console.log("param", this.counter, ":", param);
					// console.log("paramTokens", this.counter, ":", paramTokens);
					if (paramTokens && paramTokens.constructor === String) {
						// console.log("html:", paramTokens);
						param.html = paramTokens;
						eachCb(null);
					} else {
						var srcStart = paramInfo.srcOffsets[2],
							srcEnd = paramInfo.srcOffsets[3];
						if (paramInfo.spc) {
							srcStart += paramInfo.spc[2].length;
							srcEnd -= paramInfo.spc[3].length;
						} else if (paramInfo.named) {
							// The default spacing for named arguments is one
							// space after the '='
							srcStart++;
						}
						// console.log("srcStart:", srcStart, "srcEnd:", srcEnd);
						Util.processContentInPipeline(
				...
				async.each(argInfo.paramInfos, getParamHTML.bind(this), function (err) {
					...
					chunk[0].attribs.push(new KV("data-mw-arginfo", JSON.stringify(argInfo)));
					env.dp( 'TemplateHandler._onChunk', chunk );
					// console.log("\n_onChunk", this.counter, "(c)sending chunk to accum:", JSON.stringify(chunk));
					state.accum.receiveToksFromChild({tokens: chunk});
				}.bind(this));

				// console.log("_onChunk", this.counter, "(a)returning");
				return;
			}
		} else {
	...
	env.dp( 'TemplateHandler._onChunk', chunk );
	if (state.accum) {
		// console.log("\n_onChunk", this.counter, "(b)sending chunk to accum:", JSON.stringify(chunk));
		state.accum.receiveToksFromSibling({tokens: chunk, async: true});
	} else {
		// console.log("_onChunk", this.counter, "(b)sending chunk 2:", JSON.stringify(chunk));
		cb({tokens: chunk, async: true});
	}
	// console.log("_onChunk", this.counter, "(b)returning");
};
...
TemplateHandler.prototype._onEnd = function( state, cb ) {
	...
	if (this.options.wrapTemplates) {
		...
		state.emittedFirstChunk = false;
		// console.log("\n_onEnd", this.counter , "(d)sending chunk to accum:", JSON.stringify(res));
		if (state.accum) {
...
TemplateHandler.prototype.getArgInfo = function (state) {
	...
	// console.log("getArgInfo", this.counter, "params:", JSON.stringify(params));
...
TemplateHandler.prototype.onTemplateArg = function (token, frame, cb) {
	...
	if (this.options.wrapTemplates) {
		...
		newCB = function(res) {
			...
			// console.log("onTemplateArg");
			toks = tplHandler.addEncapsulationInfo(state, toks);
			// toks[0].attribs.push(new KV("data-mw-arginfo", JSON.stringify(this.getArgInfo(state))));
			toks.push(tplHandler.getEncapsulationInfoEndTag(state));
			// console.log("onTemplateArg, toks:", toks);
			cb( {tokens: toks});
		};
	} else {
		newCB = cb;
