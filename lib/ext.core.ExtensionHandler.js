...
ExtensionHandler.prototype.register = function() {
	...
	this.nativeExtHandlers = {
		...
		"infobox": nativeExts.infobox.handleInfobox.bind(nativeExts.infobox, this.manager, this.options)
	};
...
ExtensionHandler.prototype.onExtension = function( token, frame, cb ) {
	...
	// Use native handler only for <infobox> tags inside transclusions, otherwise if <infobox>
	// is present directly in the parsed article fallback to generic extension handler
	if ( extensionName === 'infobox' && !frame.parentFrame )
	{
		nativeHandler = null;
	}
