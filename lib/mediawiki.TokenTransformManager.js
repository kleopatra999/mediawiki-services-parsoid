...
TokenAccumulator.prototype.concatTokens = function (tokens) {
	...
	if (!tokens.rank) {
		this.manager.env.dp('TokenAccumulator.concatTokens without rank', tokens);
		tokens.rank = this.manager.phaseEndRank;
	}
	if (!lastChunk) {
		this.siblingChunks.push(tokens);
	} else if (tokens.rank === lastChunk.rank) {
		lastChunk = this.siblingChunks.pop().concat(tokens);
		lastChunk.rank = tokens.rank;
		this.siblingChunks.push(lastChunk);
	} else {
		this.manager.env.dp('TokenAccumulator.concatTokens with different rank', tokens, tokens.rank);
		this.siblingChunks.push(tokens);
	}
};
...
TokenAccumulator.prototype.emitTokens = function (async) {
	...
	if (this.siblingChunks.length) {
		for (var i = 0, len = this.siblingChunks.length; i < len; i++) {
			this._callParentCB({tokens: this.siblingChunks.shift(),
				async: async || (i < len-1)});
		}
	} else {
...
TokenAccumulator.prototype.receiveToksFromChild = function ( ret ) {
	...
	if (!ret.tokens.rank) {
		this.manager.env.dp('TokenAccumulator.receiveToksFromChild without rank', ret.tokens);
...
TokenAccumulator.prototype.receiveToksFromSibling = function ( ret ) {
	ret = verifyTokensIntegrity(this.manager.env, ret, false);
	// console.warn("\nTA-" + this.uid + "; c: " + this.waitForChild + "; s: " + this.waitForSibling + " <-- from sibling: " + JSON.stringify(ret));
	...
	if (this.waitForChild) {
		...
		if (!ret.tokens.rank) {
			this.manager.env.dp('TokenAccumulator.receiveToksFromSibling without rank', ret.tokens);
			ret.tokens.rank = this.manager.phaseEndRank;
		}
