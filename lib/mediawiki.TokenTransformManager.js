...
TokenAccumulator.prototype.concatTokens = function (tokens) {
	...
	if (!tokens.rank) {
		this.manager.env.log('error/tokenaccumulator/conc/rank/none');
		tokens.rank = this.manager.phaseEndRank;
	}
	if (!this.siblingChunks.length) {
		this.siblingChunks.push(tokens);
	} else if (tokens.rank === this.siblingChunks.last().rank) {
		var lastChunk = this.siblingChunks.pop().concat(tokens);
		lastChunk.rank = tokens.rank;
		this.siblingChunks.push(lastChunk);
	} else {
		this.manager.env.log('trace/tokenaccumulator/conc/rank/differs');
		this.siblingChunks.push(tokens);
	}
	// console.log("\tconcatTokens", JSON.stringify(this.siblingChunks));
};
